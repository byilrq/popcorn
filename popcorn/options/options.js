...
function renderSignin(st){
  ...
  $('keepalive_close_delay').value = Number(current.__auto_feed_keepalive_close_delay || 20);
  const rows = (st.signinSites || []).map((x, i) => `
    <div class="signin-row" data-index="${i}">
      <label><input type="checkbox" class="signin-enabled" ${x.enabled !== false ? 'checked':''}>启用</label>
      <input class="signin-name" value="${esc(x.name || '')}" placeholder="站点名">
      <input class="signin-url" value="${esc(x.url || '')}" placeholder="https://example.com/">
...
function collect(){
  ...
  data.__auto_feed_keepalive_close_delay = Math.max(5, Math.min(120, Number($('keepalive_close_delay').value || 20)));
...
$('add_signin_site').onclick=()=>{
  ...
  st.signinSites.push({ enabled:true, name: name || new URL(url).hostname, url });
...
$('run_keepalive_now').onclick=async()=>{
  ...
  chrome.runtime.sendMessage({ type:'run_keepalive', payload:{ force:true } }, (resp)=>{
    if (resp && resp.ok) setStatus('signin_status', `已开始访问 ${resp.data?.count || 0} 个站点`);
